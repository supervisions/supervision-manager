...
<script type="text/javascript">
	$(document).ready(function(){	  
            ...
            uploader.bind('UploadComplete',function(uploader,files){
                if(null != files && files.length>0){ 
                	layer.confirm('提交整改情况成功，待监察室给出意见！', {
								btn: ['确认'] //按钮
							}, function(){//点击确认按钮调用
	...
	function saveItemProcess(obj){	
		layer.confirm('确认信息已经填写完整，并且保存？', {
			...
			$.ajax({
		        cache: true, //是否缓存当前页面
		        type: "POST", //请求类型
		        url: "<%=basePath%>vision/efficiency/jsonSaveResetItem.do",
			...
			<form id="itemInfoForm" name="itemInfoForm"
				...
				<div id="tab1" class="yw-tab">
					<table class="font16 taskTable" >
						...
						<c:if test="${Item.lasgTag==666 }">
							<tr>
								<td align="right" >是否需要问责：</td>
								<td colspan="3">
									<label>
										<input type="radio" name="status" value="0" >问责
									</label> 
									<label>
										<input type="radio" name="status" value="4" checked="checked">不问责
									</label> 
