				...
				<div id="tab1" class="yw-tab">
					<table class="font16 taskTable"  cellpadding="0" cellspacing="0">
						...
						<c:if test="${FileItemProcess != null }">
							<tr>
								<td align="right" style="height:100px;">上传资料：</td>
								<td colspan="3">
						...
						<c:if test="${XYZGProcess != null }">
							<tr>
								<td align="right" style="height:100px;">监察室意见：</td>
								<td colspan="3">
									<label>${XYZGProcess.content } </label> 
									<c:forEach var="fileItem" items="${XYZGProcess.fileList }">
											<a title="点击下载" onclick="downLoadFile('${fileItem.filePath}','${fileItem.fileName}');" style="color:blue;cursor: pointer;">${fileItem.fileName}</a>
									</c:forEach> 
								</td>		
							</tr> 
						...
						<c:if test="${FollowItemProcess != null }">
							<tr>
								<td align="right" style="height:100px;">整改情况：</td>
								<td colspan="3">
